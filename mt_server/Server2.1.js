...
music.use(async function (req, res, next) {
  ...
  if (token) {
    ...
    if (req.body.name.length > 0 && req.body.password.length > 0) {
      var x = await User.findOne({ name: req.body.name }).exec();
      console.log(x);
...
music.post('/authenticateOLD', function (req, res) {
  // User
  var User = AppModels('User');
  // find the user
  User.findOne({
...
music.get('/song', async function (req, res) {
  try {
    songID = req.query.id;
    //Song file path
    var song = await Song.findOne({ id: songID }).exec();
    console.log(song)
    filePath = __dirname + '/Songs/' + song.fl;
